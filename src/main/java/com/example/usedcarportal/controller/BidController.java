...
public class BidController {
	...
	// ✅ Prevent 404 if user goes to /bids (without carId)
	@GetMapping
	...
	// ✅ Fix: Handle bid submission correctly
	@PostMapping("/submit") // ✅ Fixed path
	public String submitBid(@RequestParam Long carId, @RequestParam double bidAmount, Model model,
			Principal principal) {
		if (principal == null) {
		...
		// ✅ Correctly return bids.html instead of redirecting
		return "bids";
	}

	// ✅ Display bids for a specific car
	@GetMapping("/{carId}")
	public String viewBidsForCar(@PathVariable Long carId, Model model, Principal principal) {
		Car car = carRepository.findById(carId).orElse(null);
