...
public class AppointmentService {
	...
	public AppointmentService(AppointmentRepository appointmentRepository, BidRepository bidRepository,
			CarRepository carRepository, TransactionRepository transactionRepository) {
		...
		if (appointment != null && bid != null) {
			...
			if (car != null) {
				...
				if (bid.getBidAmount() >= car.getPrice()) {
					...
					Transaction transaction = new Transaction(car.getId(), car.getPostedBy(), bid.getBidderUsername(),
							bid.getBidAmount());
	    ...
	    for (Appointment appointment : appointments) {
	        ...
	        if (!carAppointmentMap.containsKey(appointment.getCarId()) ||
	                (appointment.getHighestBid() != null &&
	                        appointment.getHighestBid().getBidAmount() > carAppointmentMap.get(appointment.getCarId()).getHighestBid().getBidAmount())) {
